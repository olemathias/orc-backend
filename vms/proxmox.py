...
def create_qemu_vm_job(vm_id, pve_node_name):
    ...
    ip4 = "{}/{}".format(vm.config['config']['ipv4']['ip'], vm.config['config']['ipv4']['prefixlen'])
    ip6 = "{}/{}".format(vm.config['config']['ipv6']['ip'], vm.config['config']['ipv6']['prefixlen'])
    gw4 = vm.config['config']['ipv4']['gw']
    gw6 = vm.config['config']['ipv6']['gw']
    ...
    new_vm.config.post(
        ...
        net0="virtio,bridge={},tag={}".format(vm.environment.config['proxmox']['nodes'][pve_node_name]['vmbridge'], vm.config['config']['vlan_id']),
